/* hwp_server.c -- Server for hardware watchpoint handling
   ...
   This file is part the advanced debug unit / bridge.
 ...
 */
#define HWP_MAX_WP MAX_BREAKPOINTS
...
int hwp_init(int portNum)
{
   ...
   * Read all BreakPoint-Control-Registers, mark which are present.*/
  status = 0;
  for(i = 0; i < HWP_MAX_WP; i++)
    {
      regaddr = DBG_BPCTRL(i);
      errcode = dbg_cpu0_read(regaddr, &tmp);
      if(errcode != APP_ERR_NONE)
	{
	  fprintf(stderr, "ERROR reading BPCTRL%i at startup! %s\n", i,  get_err_string(errcode));
	  hwp_set_in_use(i, 1);
	}
      else
	{
	  if(tmp & DBG_BPCTRL_IMPLEMENTED)  /* HWP present */
	    {
	      hwp_set_in_use(i, 0);
	      status++;
	    }
...
void hwp_write_reg (struct rsp_buf *buf)
{
  ...
  if((regnum >= DBG_BPCTRL(0)) && (regnum <= DBG_BPCTRL(MAX_BREAKPOINTS)) && ((regnum & 0x1) == 0))
    {
      bpctrl_idx = (regnum - DBG_BPCTRL(0))/2;
      /* If the 'compare condition' (cc) or 'enable' (ena) are 0,
       * then matching is disabled and we can mark this HWP not in use.
       */ 
      cc  = val & DBG_BPCTRL_CC_MASK;
      ena = val & DBG_BPCTRL_ENABLED;
      if ((cc == 0) || (ena == 0) )
	hwp_set_in_use(bpctrl_idx, 0);
      else
	hwp_set_in_use(bpctrl_idx, 1);
    }
