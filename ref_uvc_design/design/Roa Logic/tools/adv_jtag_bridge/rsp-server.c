...
Copyright (C) 2016 Richard Herveille <richard.herveille@roalogic.com>
...
Updated for RV11 by Richard Herveille
...
#define RISCV_RSP_SERVICE  "jtag-rsp"

/*! Protocol used  */
#define RISCV_RSP_PROTOCOL  "tcp"

/* Total Number of Registers, 32GPRs, 1 PC, 32FPRs  */
#define NUM_REGS 65

/* Indices of GDB registers that are not GPRs. Must match GDB settings! */
#define NPC_REGNUM (MAX_GPRS     )	/*!< Next PC */
#define FPR_REGNUM (NPC_REGNUM +1)	/*!< Total GDB registers */

/* STALL, UNSTALL defines */
#define STALL   1
#define UNSTALL 0


/*! Definition of GDB target signals. Data taken from gdb/signals.def
    Only those we use defined here. */
enum target_signal {
  GDB_SIGNAL_NONE =  0, //None
  GDB_SIGNAL_INT  =  2, //Interrupt
  GDB_SIGNAL_ILL  =  4, //Illegal
  GDB_SIGNAL_TRAP =  5, //Trace/Breakpoint
  GDB_SIGNAL_FPE  =  8, //Arithmetic Exception
  GDB_SIGNAL_BUS  = 10, //Bus Error
  GDB_SIGNAL_SEGV = 11, //Segmentation Fault
  GDB_SIGNAL_ALRM = 14, //Alarm Clock (Timer Tick)
  GDB_SIGNAL_USR1 = 30,
  GDB_SIGNAL_USR2 = 31,
  GDB_SIGNAL_PWR  = 32
};
...
/*!Initialize the Remote Serial Protocol connection
   ...
   in the constant RISCV_RSP_SERVICE.

   The protocol used for communication is specified in RISCV_RSP_PROTOCOL. */
/*---------------------------------------------------------------------------*/
void
rsp_init (int portNum)
{
  ...
  if (0 == portNum)
	...
	getservbyname (RISCV_RSP_SERVICE, protocol->p_name);

      if (NULL == service)
	{
	  fprintf (stderr, "Warning: RSP unable to find service \"%s\": %s\n",
		   RISCV_RSP_SERVICE, strerror (errno));
	  return;
	}
  ...
  switch (poll (fds, 2, -1))
      ...
      else if(POLLIN == (fds[1].revents & POLLIN))
	{
	  ...
	  if(monitor_status == 'H')
		...
		{
                  debug("HWP server reports [H]alted\n");

		  rsp.target_running = 0;
		  // Log the exception so it can be sent back to GDB
		  dbg_cpu0_read(DBG_CAUSE, &causeval);  // Read DBG_CAUSE; find out why we stopped
		  rsp_exception(causeval);  // Send it to be translated and stored
		  ...
		  if (rsp.client_waiting)
		    { 
		      // Read the PPC
		      dbg_cpu0_read(DBG_PPC, &ppcval);
		    
		      // This is structured the way it is to avoid reading the target unless it's necessary.
		      if (GDB_SIGNAL_TRAP == rsp.sigval)
			{
			  if(NULL != mp_hash_lookup (BP_MEMORY, ppcval))  // Is this a breakpoint we set? (we also get a TRAP on single-step)
			    ...
			    {
			      uint32_t dbg_hitval;
			      dbg_cpu0_read(DBG_HIT, &dbg_hitval);      // We need this to check for a hardware breakpoint
			      if((dbg_hitval & DBG_HIT_BP_MASK) != 0)   // Is this a hardware breakpoint?
				{	  
				  //fprintf(stderr, "Resetting NPC to PPC\n");
				  //Do not reset NPC for the RoaLogic Debug Unit
				  //set_npc(ppcval);
				}
			    }
			}
...
//   future call to handle_exception will report it back to the client. 
...
void rsp_exception (unsigned long int except)
{
  ...
  //Check if any of the HIT bits is set
  dbg_cpu0_read(DBG_HIT, &tmp); 

  if (tmp != 0) {
    //We got a 'hit' ... GDB expects a TRAP exception
    //which, for HW performance reasons, is not triggered
    sigval = GDB_SIGNAL_TRAP;
  }
  else if ( (int)except == -1) {
    //no idea ... we wrote '-1' into the DBG_CAUSE register
    //we ended up here without a HIT or exception. Somebody pressed 'stop/pauze'?
    sigval = GDB_SIGNAL_NONE;
  }
  else if (except > 31) {
    //Any value larger than 15 indicates an interrupt (XLEN set)
    if (except == CSR_CAUSE_TIMER_INT) {
      //special case for timer-tick
      sigval = GDB_SIGNAL_ALRM;
    } else {
      sigval = GDB_SIGNAL_INT;
    }
  } else {
    switch (except) {
      case CSR_CAUSE_INST_MISALIGNED    : sigval = GDB_SIGNAL_BUS;  break;
      case CSR_CAUSE_INST_ACCESS_FAULT  : sigval = GDB_SIGNAL_SEGV; break;
      case CSR_CAUSE_ILLEGAL            : sigval = GDB_SIGNAL_ILL;  break;
      case CSR_CAUSE_BREAKPOINT         : sigval = GDB_SIGNAL_TRAP; break;
      case CSR_CAUSE_LOAD_MISALIGNED    : sigval = GDB_SIGNAL_BUS;  break;
      case CSR_CAUSE_LOAD_ACCESS_FAULT  : sigval = GDB_SIGNAL_SEGV; break;
      case CSR_CAUSE_AMO_MISALIGNED     : sigval = GDB_SIGNAL_BUS;  break;
      case CSR_CAUSE_STORE_ACCESS_FAULT : sigval = GDB_SIGNAL_SEGV; break;
      case CSR_CAUSE_UMODE_ECALL        : sigval = GDB_SIGNAL_TRAP; break; //RiH: Is ECALL another trap??
      case CSR_CAUSE_SMODE_ECALL        : sigval = GDB_SIGNAL_TRAP; break;
      case CSR_CAUSE_HMODE_ECALL        : sigval = GDB_SIGNAL_TRAP; break;
      case CSR_CAUSE_MMODE_ECALL        : sigval = GDB_SIGNAL_TRAP; break;
...
rsp_server_request ()
{
  ...
  dbg_cpu0_write(DBG_CAUSE, -1);
  dbg_cpu0_read(DBG_IE, &tmp);
  dbg_cpu0_write(DBG_IE, tmp | DBG_IE_BREAKPOINT | DBG_IE_ILLEGAL | DBG_IE_INST_MISALIGNED | DBG_IE_LOAD_MISALIGNED | DBG_IE_AMO_MISALIGNED);
}	/* rsp_server_request () */
...
rsp_client_request ()
{
  ...
  switch (buf->data[0])
    ...
    case 'D':
	 ...
	 execution should continue. TODO. Is this really the intended
	 meaning? Or does it just mean that only vAttach will be recognized
	 after this? */
      put_str_packet ("OK");
      rsp_client_close ();
      set_stall_state (UNSTALL);
      return;
...
{ 
  ...
  // If target is running, stop it so we can modify CSRs
  if(rsp.target_running) {
    set_stall_state(STALL);
  }

  // Clear the Debug Registers: don't transfer control to the debug unit for any reason
  dbg_cpu0_write(DBG_CTRL, 0);
  for (i=0; i<MAX_BREAKPOINTS; i++)
    dbg_cpu0_write(DBG_BPCTRL(i),0);
...
/*!Set the program counter
   ...
   Not completely trivial, since this is actually cached in cpu_state.pc.
   Any reset of the NPC also involves clearing the delay state and setting
   the pcnext global.
...
static unsigned int set_npc (unsigned long int  addr)
{
  ...
  errcode = dbg_cpu0_write(DBG_NPC, addr);
...
rsp_continue (struct rsp_buf *buf)
{
  ...
  rsp_continue_generic (NO_VECTOR);
...
/*!Generic processing of a continue request
   ...
   The signal may be NO_VECTOR if there is no exception to be
...
{
  /* Clear Single-Step-Trace and Branch-Trace triggers */
  dbg_cpu0_write(DBG_CTRL, 0);

  /* Clear any breakpoint hits */
  dbg_cpu0_write(DBG_HIT, 0);
...
/*!Handle a RSP read all registers request
   ...
   The registers follow the GDB sequence for RISC-V GDB;
...
rsp_read_all_regs ()
{
  ...
  uint32_t        regbuf[MAX_GPRS];	/* should be 64bit for RV64. TODO */
  unsigned int    errcode = APP_ERR_NONE;

  //CPU must be stall to access any registers
  if (rsp.target_running) {
    set_stall_state(STALL);
  }
  ...
  errcode = dbg_cpu0_read_block(DBG_GPR_BASE, regbuf, MAX_GPRS);

  /* Format the GPR data for output */
  for (r = 0; r < MAX_GPRS; r++) reg2hex(regbuf[r], &(buf.data[r * 8]));

  /* NPC */
  errcode |= dbg_cpu0_read(DBG_NPC, regbuf);

  // Note that reg2hex adds a NULL terminator
  reg2hex(regbuf[0], &(buf.data[NPC_REGNUM * 8]));

  // Read all FPRs in a single burst
  errcode |= dbg_cpu0_read_block(DBG_FPR_BASE, regbuf, MAX_FPRS);

  /* Format the FPR data for output */
  for (r = 0; r < MAX_FPRS; r++) reg2hex(regbuf[r], &(buf.data[(FPR_REGNUM + r) * 8]));


  //if CPU was running before entering function, start it again
  if (rsp.target_running) {
    set_stall_state(UNSTALL);
  }
...
{
  int             r;			/* Register index */
  uint32_t        regbuf[MAX_GPRS];	/* TODO should be 64bit for RV64 */
  unsigned int    errcode;

  /* The GPRs */

  // Set up the GPR data for a burst access
  for (r = 0; r < MAX_GPRS; r++) regbuf[r] = hex2reg (&(buf->data[r * 8]));

  errcode = dbg_cpu0_write_block(DBG_GPR_BASE, regbuf, MAX_GPRS);

  /* NPC */
  regbuf[0] = hex2reg (&(buf->data[NPC_REGNUM * 8]));

  errcode |= dbg_cpu0_write(DBG_NPC, regbuf[0]);

  /* The FPRs */
  for (r = 0; r < MAX_FPRS; r++) regbuf[r] = hex2reg (&(buf->data[FPR_REGNUM + r * 8]));

  errcode = dbg_cpu0_write_block(DBG_FPR_BASE, regbuf, MAX_FPRS);
...
rsp_read_reg (struct rsp_buf *buf)
{
  ...
  if (regnum < MAX_GPRS)
    {
      //GPRs
      errcode = dbg_cpu0_read(DBG_GPR_BASE + regnum, &tmp);
    }
  else if (NPC_REGNUM == regnum)
    {
      //NPC
      errcode = dbg_cpu0_read(DBG_NPC, &tmp);
    }
  else if (regnum < NUM_REGS)
    {
      //FPRs
      errcode = dbg_cpu0_read(DBG_FPR_BASE + regnum - FPR_REGNUM, &tmp);
    }
  else
    {
...
rsp_write_reg (struct rsp_buf *buf)
{
  ...
  if (regnum < MAX_GPRS)
    {
      //GPRs
      errcode = dbg_cpu0_write(DBG_GPR_BASE + regnum, hex2reg(valstr));
    }
  else if (NPC_REGNUM == regnum)
    {
      //NPC
    ...
    {
      //FPRs
      errcode = dbg_cpu0_write(DBG_FPR_BASE + regnum - FPR_REGNUM, hex2reg(valstr));
    }
  else
    {
      /* Error response if we don't know the register */
...
rsp_command (struct rsp_buf *buf)
{
  ...
  if (0 == strncmp ("readcsr ", cmd, strlen ("readcsr")))
    {
      unsigned int       regno;

      /* Parse and return error if we fail */
      if( 1 != sscanf (cmd, "readcsr %4x", &regno))
	{
      ...
      if (regno > MAX_CSRS)
	{
      ...
      /* Construct the reply */
      dbg_cpu0_read(DBG_CSR_BASE + regno, &tmp);  // TODO Check return value of all hardware accesses
      sprintf (cmd, "%8x", tmp);
      ascii2hex (buf->data, cmd);
      buf->len = strlen (buf->data);
      put_packet (buf);
    }
  else if (0 == strncmp ("writecsr ", cmd, strlen ("writecsr")))
    {
      unsigned int       regno;
      unsigned long int  val;

      /* Parse and return error if we fail */
      if( 2 != sscanf (cmd, "writecsr %4x %8lx", &regno, &val))
	{
	  fprintf (stderr, "Warning: qRcmd %s not recognized: ignored\n",
		   cmd);
	  put_str_packet ("E01");
      ...
      if (regno > MAX_CSRS)
	{
	  fprintf (stderr, "Warning: qRcmd writecsr %x too large: ignored\n",
		   regno);
	  put_str_packet ("E01");
	  return;
	}

      /* Update the CSR and reply "OK" */
      dbg_cpu0_write(DBG_CSR_BASE + regno, val);
      put_str_packet ("OK");
    }
...
/*!Handle a RSP step request
   ...
   Parse the command to see if there is an address.
   Uses the underlying generic step function.
...
rsp_step (struct rsp_buf *buf)
{
  ...
  if(strncmp(buf->data, "s", 2))
	...
	{
          debug("Single Step with address (%.8x)\n", addr);
	  /* Set the address as the value of the next program counter */
  ...
  rsp_step_generic (0); //is EXCEPT actually used?!
...
rsp_step_generic (unsigned long int  except)
{
  ...
  /* Clear SingleStep-Hits and enable the
     single step trigger in MDCTRL
     Enable traps to be handled by the debug unit  */
  if(!rsp.single_step_mode)
    { 
      dbg_cpu0_write(DBG_HIT, 0);
      dbg_cpu0_write(DBG_CTRL, DBG_CTRL_SINGLE_STEP_TRACE);
...
rsp_remove_matchpoint (struct rsp_buf *buf)
{
  ...
  switch (type)
    {
    case BP_MEMORY:
      debug ("Remove BREAK instruction @%.8x\n", addr);
	...
	{
	  instbuf[0] = mpe->instr;
	  dbg_wb_write32(addr, instbuf[0]);  // *** TODO Check return value
	  free (mpe);
	}
    ...
    case WP_ACCESS:
      mpe = mp_hash_delete (type, addr);
      /* The hwp we used is stored in mpe->instr */
      if(NULL != mpe)
	{
	  hwp = mpe->instr;
	  free (mpe);

          debug("Removing watchpoint-%0d\n", hwp);

          /* disable HW BP */
          dbg_cpu0_write(DBG_BPCTRL(hwp), 0 );

	  /* Clear hit bits in DBG_HIT */
	  regaddr = DBG_HIT;
          dbg_cpu0_read(regaddr, &regdata);
          regdata &= ~DBG_HIT_BP(hwp);
...
rsp_insert_matchpoint (struct rsp_buf *buf)
{
  ...
  switch (type)
    {
    case BP_MEMORY:
      debug("Insert BREAK instruction @%.8x\n", addr);
      ...
      instbuf[0] = RISCV_EBREAK_INSTR;         // Set the EBREAK instruction
      dbg_wb_write32(addr, instbuf[0]);           // *** TODO Check return value
    ...
    case BP_HARDWARE:
      debug("Setting BP_HARDWARE breakpoint @%.8x\n", addr);
      hwp = hwp_get_available_watchpoint();
      debug("Got wp %i from HWP\n", hwp);
      ...
      else
	{
	  ...
	  regaddr = DBG_BPDATA(hwp);
	  dbg_cpu0_write(regaddr, addr);     // *** TODO Check return value

	  /* Set Breakpoint Control Register */
	  regaddr = DBG_BPCTRL(hwp);
          regdata = DBG_BPCTRL_ENABLED | DBG_BPCTRL_CC_INST_FETCH;
	  dbg_cpu0_write(regaddr, regdata);  // *** TODO Check return value
      ...
      else
	{
	  ...
	  /* Set Breakpoint Control Register */
	  regaddr = DBG_BPCTRL(hwp);
          regdata = DBG_BPCTRL_ENABLED | DBG_BPCTRL_CC_ST_ADR;
      ...
      else
	{
	  ...
	  /* Set Breakpoint Control Register */
	  regaddr = DBG_BPCTRL(hwp);
          regdata = DBG_BPCTRL_ENABLED | DBG_BPCTRL_CC_LD_ADR;
      ...
      else
	{
	  ...
	  /* Set Breakpoint Control Register */
	  regaddr = DBG_BPCTRL(hwp);
          regdata = DBG_BPCTRL_ENABLED | DBG_BPCTRL_CC_LDST_ADR;
...
void set_stall_state(int stall)
{
  ...
  if(stall) {
      debug("RSP server requests [S]tall\n");
      ret = write(pipe_fds[0], "S", 1); 
  } else {
      debug("RSP server requests [U]nstall\n");
      ret = write(pipe_fds[0], "U", 1); 
  }
