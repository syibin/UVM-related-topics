...
int adbg_ctrl_write(unsigned long regidx, uint32_t *cmd_data, int length_bits) {
  ...
  case DC_CPU0:
    ...
    cpu_select_len = DBG_CPU_SELECT_LEN;
debug("index_len:%0d, opcode_len:%0d, opcode:%0x,cpu_select_len=%0d\n",index_len,opcode_len,opcode,cpu_select_len);
    break;
  case DC_CPU1:
    index_len = DBG_CPU1_REG_SEL_LEN;
  ...
  data = ~(1 << (opcode_len + cpu_select_len + index_len));             //MSB must be 0 to access modules
  data &= (((opcode << cpu_select_len) | cpu ) << index_len) | regidx;
debug("data=%x\n",data);
debug("cmd_data=%x, length:%0d\n", *cmd_data, length_bits);
  err |= tap_set_shift_dr();  /* SHIFT_DR */ 
  
  /* write data, EXIT1_DR */
  err |= jtag_write_stream(cmd_data, length_bits, 0);
  err |= jtag_write_stream(&data, 1+opcode_len+cpu_select_len+index_len, 1);
...
int adbg_ctrl_read(unsigned long regidx, uint32_t *data, int databits) {
  ...
  outdata[0] = ~(1 << (opcode_len + cpu_select_len)); // Zero MSB = op for module, not top-level debug unit
  outdata[0] &= (opcode << cpu_select_len) | cpu;       // Set opcode, select CPU
  ...
  err |= jtag_write_stream(outdata, opcode_len+cpu_select_len+1, 1);  // adjust for postfix bits, Set TMS: EXIT1_DR
 ...
 */
int adbg_burst_command(unsigned int opcode, unsigned int opcode_len, unsigned long address, unsigned int address_len, unsigned int cpu, unsigned int cpu_select_len, int length_words) {
  ...
  // get total bitcount
  bit_cnt  = 1;                 // 1-bit module command
  bit_cnt += opcode_len;        // opcode bits
  bit_cnt += cpu_select_len;    // cpu-select bits
  bit_cnt += address_len;       // address bits
  bit_cnt += 16;                // burst length
  ...
  if (address_len == 32) {
    ...
    data[1] = opcode;
    if (cpu_select_len) {
      data[1] = (data[1] << cpu_select_len) | cpu;
    }
    data[1] = (data[1] << 16) | (address >> 16);
    data[1] &= ~(1 << (16 + cpu_select_len + opcode_len)); //MSB must be 0 to access modules
  }
  else
  {
    //64bit address ...
    ...
    data[2] = opcode;
    if (cpu_select_len) {
      data[2] = (data[2] << cpu_select_len) | cpu;
    }
    data[2] = (data[2] << 16) | (address >> 48);
    data[2] &= ~(1 << (16 + cpu_select_len + opcode_len));
  }

  err |= tap_set_shift_dr();  /* SHIFT_DR */ 
  
  /* write data, EXIT1_DR */
  err |= jtag_write_stream(data, bit_cnt, 1);  // When TMS is set (last parameter), DR length is also adjusted; EXIT1_DR
...
int adbg_wb_burst_read(int word_size_bytes, int word_count, unsigned long start_address, void *data)
{
  ...
  //rih: for now fix address length to 32bits, fix CPU to 0
  addr_len = 32;
  cpu = 0;

    debug("Doing burst read, word size %d, word count %d, %0dbit start address 0x%lX\n", word_size_bytes, word_count, addr_len, start_address);
    ...
    case DC_WISHBONE:
      ...
      else if(word_size_bytes == 8) opcode = DBG_WB_CMD_BREAD64;
      else {
 ...
 wb_burst_read_retry_partial:
    ...
    */
    //rih
    if(err |= adbg_burst_command(opcode, opcode_len, addr, addr_len, cpu, cpu_select_len, (word_count-i)))
      return err; 
...
int adbg_wb_burst_write(void *data, int word_size_bytes, int word_count, unsigned long start_address)
{
  ...
  cpu = 0;
    ...
    debug("Doing burst write, word size %d, word count %d, %0dbit start address 0x%lx\n", word_size_bytes, word_count, addr_len, start_address);
    word_size_bits = word_size_bytes << 3;
    ...
    case DC_WISHBONE:
      ...
      else if(word_size_bytes == 8) opcode = DBG_WB_CMD_BWRITE64;
      else {
